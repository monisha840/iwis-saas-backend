...
// Models that own a hospitalId column (12 original + 28 anchor tables) — derived
// from the schema so it stays correct as the schema evolves.
  ...
  query: {
    $allModels: {
      async $allOperations({ model, operation, args, query }) {
        ...
        // No tenant (SUPER_ADMIN / jobs) or non-scoped model → pass through.
        if (!tenant || !SCOPED_MODELS.has(model)) return query(args);
