...
router.get('/list-therapists', authMiddleware, roleMiddleware(['ADMIN', 'ADMIN_DOCTOR', 'BRANCH_ADMIN']), validate({ query: listTherapistsSchema }), async (req, res, next) => {
  try {
    const { search } = req.query;
    // BRANCH_ADMIN is hard-pinned to their JWT branch — query param is ignored
    // so it can't be used to peek at staff in another branch.
    const branchId = req.user.role === 'BRANCH_ADMIN' ? req.user.branchId : req.query.branchId;
...
router.get('/list-doctors', authMiddleware, roleMiddleware(['ADMIN', 'ADMIN_DOCTOR', 'BRANCH_ADMIN']), validate({ query: listDoctorsSchema }), async (req, res, next) => {
  try {
    const { search } = req.query;
    const branchId = req.user.role === 'BRANCH_ADMIN' ? req.user.branchId : req.query.branchId;
